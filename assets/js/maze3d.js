(function() {
    ...
    function initializeScene() {
        ...
        floorplanImage.onload = function() {
            ...
            console.log("平面图加载成功！");
            drawMiniMapStatic(); // 图片加载完后重新绘制
        };
        floorplanImage.onerror = function() {
            console.warn("平面图加载失败，使用默认格子绘制");
            floorplanLoaded = false;
            drawMiniMapStatic();
        };
        // ⚠️ 修改这里的路径为你的平面图路径
        floorplanImage.src = "assets/images/floorplan.png";
    ...
    function drawMiniMapStatic() {
        ...
        if (floorplanLoaded) {
            // ✨ 使用平面图作为底图
            ctx.drawImage(floorplanImage, 0, 0, w, h);
    ...
    function updateMiniMapOverlay() {
        ...
        // 玩家点 - 使用更醒目的颜色
        ctx.fillStyle = "#00FF00"; // 绿色（也可以用 "#FF0000" 红色）
        ctx.beginPath();
        ctx.arc(px * mapScale, py * mapScale, 4, 0, Math.PI * 2);
        ctx.fill();
